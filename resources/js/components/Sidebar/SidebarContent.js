...
class SideBarContent extends Component {
	...
	renderCalls() {
		...
		if(callsLoading){
			return (
				<div className={classes.loadingOffset}>
					<Loading />
				</div>
			);
		} else {
		...
		return (
			<List>
				{contacts.map((contact, index) => {
					return (
						<ListItem className={classes.listItemCall} button key={`contact-${index}`}>
							<ListItemText primary={`${contact.get('name')}`} secondary={contact.get('email')} />
							<ListItemSecondaryAction>
								<IconButton onClick={() => this.props.call(contact.get('id'), contact.get('name'))} color="primary"><Call /></IconButton>
							</ListItemSecondaryAction>
						</ListItem>
					)
					})}
